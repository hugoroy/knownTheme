<?php
    ...
    namespace Themes\Hugoroy\Pages {
        ...
        class Admin extends \Idno\Common\Page
        {
            ...
            function getContent()
            {
                ...
                $t->body  = $t->__(array('users' => $users))->draw('admin/hugoroy');
            ...
            function postContent()
            {
                ...
                if ($profile_user = $this->getInput('profile_user')) {
                    \Idno\Core\site()->config->config['hugoroy']['profile_user'] = $profile_user;
                }
                if (!empty($_FILES['background']) && $this->getInput('action') != 'clear') {
                        ...
                        if (getimagesize($_FILES['background']['tmp_name'])) {
                            if ($background = \Idno\Entities\File::createFromFile($_FILES['background']['tmp_name'], $_FILES['background']['name'])) {
                                ...
                                if (!empty(\Idno\Core\site()->config()->hugoroy['bg_id'])) {
                                    if ($file = File::getByID(\Idno\Core\site()->config()->hugoroy['bg_id'])) {
                                ...
                                \Idno\Core\site()->config->config['hugoroy']['bg_id'] = $background;
                                $background = \Idno\Core\site()->config()->getDisplayURL() . 'file/' . $background;
                                \Idno\Core\site()->config->config['hugoroy']['bg'] = $background;
                            }
                        }
                    }
                } else {
                    ...
                    \Idno\Core\site()->config->hugoroy = [];
                }
                \Idno\Core\site()->config->save();
                $this->forward(\Idno\Core\site()->config()->getDisplayURL() . 'admin/hugoroy/');
            }
